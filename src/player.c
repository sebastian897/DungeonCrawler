  ...
  if (is_moving) {
    ...
    for (int my = 0; my < map_max_height; my++) {
      for (int mx = 0; mx < map_max_width; mx++) {
        ...
        V2 tile = {mx, my};
        bool xcol = Collided((Rec){{newx, p->rec.pos.y}, p->rec.size}, GetTileRec(tile));
        bool ycol = Collided((Rec){{p->rec.pos.x, newy}, p->rec.size}, GetTileRec(tile));
        if (xcol) {
          x_col_tile = true;
    ...
    for (int at = 0; at < ARRAY_LENGTH(anim_types); at++) {
      ...
      //   p->character.hand_anim.rot = p->rot;
      // }
    }
  } else {
    for (int at = 0; at < ARRAY_LENGTH(anim_types); at++) {
...
static void RotAttackAnim(Player* player) {
  ...
  }  // if (player->weapon.effect_anim.anim_action == aa_attacking) player->weapon.effect_anim.rot =
     // rot;
}
...
void AnimatePlayer(Player* player) {
  for (int at = 0; at < ARRAY_LENGTH(anim_types); at++) {
    ...
    // GoToNextSprite(&player->character.anims[at], player);
    // GoToNextSprite(&player->weapon.effect_anim, player);
