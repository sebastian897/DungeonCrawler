  ...
  if (!(dir_vec.x == 0 && dir_vec.y == 0)) {
    ...
    for (int my = 0; my < map_max_height; my++) {
      for (int mx = 0; mx < map_max_width; mx++) {
        ...
        if (xcol) x_col_tile = true;
        if (ycol) y_col_tile = true;
        if ((xcol || ycol)) {
          col_tile = DeepCopyV2(SubV2(tile, dir_vec));
        }
      }
    }
    if (!x_col_tile) {
      p->pos.x = newx;
    } else if (col_tile.x >= 0 && !y_col_tile) {
      p->pos.x = V2ToScreenPos(col_tile).x;
    }
    if (!y_col_tile) {
      p->pos.y = newy;
    } else if (col_tile.y >= 0 && !x_col_tile) {
...
int main(void) {
  ...
  V2 tile = {0, 2};
  bool xcol = Collided((ScreenPos){59, 150}, V2ToScreenPos(tile));
  bool ycol = Collided((ScreenPos){64, 150}, V2ToScreenPos(tile));
  int zcol = xcol;
  ...
  while (!WindowShouldClose()) {
    ...
    // DrawText(text, 0, 0, tile_size, BLACK);
